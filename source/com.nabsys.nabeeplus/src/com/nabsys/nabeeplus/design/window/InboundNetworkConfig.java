...
public class InboundNetworkConfig extends ConfigPopupWindow{
	
	private SearchPopupList 	popupList 					= null;
	
	public InboundNetworkConfig(Shell parent) {
		super(parent);
	}

	public HashMap<String, Object> open(IWorkbenchWindow window, Image icon) {
		super.open(window, icon, "Inbound Network", new Point(400, 400));
		...
		FieldFrame fieldFrame = new FieldFrame(contentsBack, SWT.NONE, 100);
		fieldFrame.setLayoutData(layoutData);

		final NCombo cmbProtocol = fieldFrame.getComboField("PI", NBLabel.get(0x015D));
		setProtocolList(cmbProtocol);
		cmbProtocol.addNBModifiedListener(new NBModifiedListener(){
			public void modified(String name, String value) {
				returnMap.put(name, value.replaceAll("\\].*", "").replaceAll("\\[", ""));
			}
		});

		if(returnMap.get("PI") != null && !((String)returnMap.get("PI")).equals(""))
		{
		...
		else if(cmbProtocol.getItemCount() > 0)
		{
		...
		cmbProtocol.addMouseListener(mouseListener);
	...
	private void setProtocolList(NCombo cmbProtocol)
	{
		NBFields fields = new NBFields();
