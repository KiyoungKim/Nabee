...
public class DatabaseSelectConfig extends ConfigPopupWindow{

	private SearchPopupList 	popupList 					= null;
	
	public DatabaseSelectConfig(Shell parent) {
		super(parent);
	}
	
	public HashMap<String, Object> open(IWorkbenchWindow window, Image icon) {
		super.open(window, icon, "Database Select", new Point(480, 350));
		...
		final NStyledText txtListName = fieldFrame.getTextField("LN", NBLabel.get(0x0272));
		txtListName.addNBModifiedListener(new NBModifiedListener(){
			public void modified(String name, String value) {
				returnMap.put(name, value);
			}
		});
		
		if(returnMap.get("LN") != null && !((String)returnMap.get("LN")).equals(""))
		{
			txtListName.setText((String)returnMap.get("LN"));
		}
		
		NCheckBox chkHasList = fieldFrame.getCheckField("CHL", NBLabel.get(0x0275), NBLabel.get(0x0276), NBLabel.get(0x0277));
		chkHasList.addNBModifiedListener(new NBModifiedListener(){
			public void modified(String name, String value) {
				boolean isOneRow =  value.equals("true");
				if(isOneRow)
				{
					txtListName.setText("");
					txtListName.setEditable(false);
					txtListName.setBackground(new Color(display, 240, 240, 240));
					txtListName.setForeground(new Color(display, 150, 150, 150));
				}
				else
				{
					txtListName.setEditable(true);
					txtListName.setBackground(display.getSystemColor(SWT.COLOR_WHITE));
					txtListName.setForeground(display.getSystemColor(SWT.COLOR_BLACK));
				}
				returnMap.put(name, isOneRow);
			}
		});
		
		if(returnMap.get("CHL") != null)
		{
			boolean isOneRow =  (Boolean)returnMap.get("CHL");
			chkHasList.setSelection(isOneRow);
		...
		NCheckBox chkSetReturn = fieldFrame.getCheckField("ISR", NBLabel.get(0x0278), NBLabel.get(0x0266), NBLabel.get(0x0267));
